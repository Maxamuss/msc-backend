...
from ..exceptions import InvalidFieldNameError, NullFieldChangedError, OutdatedModelError
from ..models import FieldSchema, ModelSchema


class TestModelSchema(TestCase):
    def test_create_model(self):
        model_schema = ModelSchema.objects.create(name='Test')
        model_class = model_schema.as_model()

        # Test default fields added to new model.
        expected_fields = ['id', 'created_at', 'updated_at']
        self.assertListEqual([x.name for x in model_class._meta.fields], expected_fields)

        # Test new model can have instanced created.
        ...
        # Test field can be added to model.
        ...
        print(model_schema.fields.all())

        self.assertTrue('test_field' in model_class._meta.fields)
